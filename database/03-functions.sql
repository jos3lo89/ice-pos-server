...
-- Función para calcular totales de la orden (Contexto Perú - IGV 18%)
CREATE OR REPLACE FUNCTION update_order_totals(p_order_id INTEGER)
RETURNS VOID AS $$
DECLARE
...
BEGIN
    -- Obtener tasa de IGV (por defecto 18)
    SELECT COALESCE(value::DECIMAL, 18) INTO v_igv_rate FROM settings WHERE key = 'igv_rate';

    -- Suma de todos los items que no estén cancelados
    SELECT COALESCE(SUM(unit_price * quantity), 0) INTO v_total_items
    FROM order_items WHERE order_id = p_order_id AND status != 'cancelado';

    -- En Perú el precio suele incluir IGV, así que desglosamos:
    v_subtotal_neto := v_total_items / (1 + (v_igv_rate / 100));
    v_igv_amount := v_total_items - v_subtotal_neto;

    -- Obtener cuánto se ha pagado hasta ahora
    ...
    -- Si ya se pagó todo, cerrar orden
    IF v_amount_paid >= v_total_items AND v_total_items > 0 THEN
        UPDATE orders SET status = 'completado', completed_at = NOW() WHERE id = p_order_id;
        -- Liberar mesa
        UPDATE tables SET status = 'disponible', current_order_id = NULL
        WHERE current_order_id = p_order_id;
    END IF;
END;
$$ LANGUAGE plpgsql;

-- Función para pagar items específicos
CREATE OR REPLACE FUNCTION pay_specific_items(
    p_order_id INTEGER,
    p_item_ids INTEGER[],
    p_method payment_method,
    p_cajero_id INTEGER,
...
BEGIN
    -- Registrar el pago
    INSERT INTO payments (order_id, cajero_id, amount, method)
    VALUES (p_order_id, p_cajero_id, p_amount, p_method)
    RETURNING id INTO v_payment_id;

    -- Marcar items como pagados y vincular al pago
    FOREACH v_item_id IN ARRAY p_item_ids
    LOOP
        UPDATE order_items SET is_paid = TRUE WHERE id = v_item_id;
        INSERT INTO payment_details (payment_id, order_item_id) VALUES (v_payment_id, v_item_id);
    END LOOP;
...
-- Función para cancelar orden con motivo
